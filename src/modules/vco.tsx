...
export const vcoTemplate = {
  ...
  ports: [
    ...
    { key: 'freq', label: 'FREQ (Hz)', direction: 'in', kind: 'param' } as PortSpec,
  ]
}

export function createVCORuntime(mod: ModuleInstance): ModuleRuntime {
  const osc = new Tone.Oscillator({ type: mod.params.waveform, frequency: mod.params.frequency, detune: mod.params.detune, volume: mod.params.volume }).start()
  const out = new Tone.Gain(1)
  osc.connect(out)
  return {
    getOut: (k) => (k === 'out' ? out : null),
    getIn: (k) => (k === 'freq' ? osc.frequency : null),
    update: (m) => { osc.type = m.params.waveform; (osc.frequency as any).value = m.params.frequency; (osc.detune as any).value = m.params.detune },
    dispose: () => { osc.disconnect(); osc.dispose(); out.dispose() }
  }
}
...
export const VCOUI: React.FC<{ mod: ModuleInstance }> = ({ mod }) => (
  <div className="col">
    ...
    <div className="jack-row">
      ...
      <Jack moduleId={mod.id} portKey="freq" label="FREQ CV" direction="in" kind="param" />
    </div>
